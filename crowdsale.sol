 ...
 */
contract BSCCROP is IBEP20, HasForeignAsset {
...
contract PrivateSale is Ownable{
  ...
  constructor(uint256 _rate, address payable _wallet, BSCCROP  _token, uint256 startTime, uint256 endTime) {
    require(_rate > 0);
  ...
  function buyTokens(address _beneficiary) public payable {
    ...
    uint256 _25Percent = tokens.div(4);
    tokenHolders[_beneficiary] = _25Percent;
    // update state
    weiRaised = weiRaised.add(weiAmount);
    
    _deliverTokens(_beneficiary, _25Percent);
    emit TokenPurchase(
      msg.sender,
      _beneficiary,
      weiAmount,
